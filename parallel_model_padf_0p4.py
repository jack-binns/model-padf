...
#    angle_dist_histogram_list = []
    ...
    n = 0
    print("Binning...")
    print("current file:", three_body_path)
...
def molecular_n_body_problem_pool(i, a_i, atoms, probe, distance_bin, path, fourbody=True):
    """
...
def parallel_histo(pool_num, parent_loop, root, angular_bin_size, dist_bin_size, dist_limit):
    ...
    ftbout = open(loop_sum_path_tb,"w")
    ffbout = open(loop_sum_path_fb,"w")
    for k in np.arange(0, int(pool_num - 1), 1):
        ...
        ftba = open( tba_path, "r")
        count = 0
        for line in ftba:
            ftbout.write(line)
            count += 1
        if (count!=0) and k!=int(pool_num-2):
            ftbout.write("\n")
        ftba.close()

        print("Summing four-body arrays for loop ", parent_loop)
        fba_path = root + "_" + str(k) + "_four_body_array.dat"
        ffba = open( fba_path, "r")
        count = 0
        for line in ffba:
            ffbout.write(line)
            count += 1
        if (count!=0) and k!=int(pool_num-2):   #ensures new line character at end of file
            ffbout.write("\n")
        ffba.close()
    ...
    tb_hist_unrefcor = tb_histogram(loop_sum_path_tb, dist_limit, angular_bin_size, dist_bin_size,
                                    True)  # calculate the histogram for three-body sum
    fb_hist_unrefcor = fb_histogram(loop_sum_path_fb, dist_limit, angular_bin_size, dist_bin_size,
...
def finalise_histograms(path, ang_bin, r_dist_bin, r_exp, loop):
...
def run(modelp):
    ...
    root = modelp.root #"/Users/andrewmartin/Work/Teaching/2020/ONPS2186/codes/model-padf-master/"

    project = modelp.project #""
    xyz_name = modelp.xyz_name #"1al1_ex.xyz"  # the xyz file contains the cartesian coords of the crystal structure expanded to include r_probe
    cif_name = modelp.cif_name #"1al1_edit.cif"  # the cif containing the asymmetric unit. This often needs to be edited for PDB CIFs hence '_edit' here

    output_path = root + project + xyz_name[:-4]

#    pool = modelp.pool #mp.Pool(mp.cpu_count())  # change to set the number of processors
    ...
    ucds = modelp.ucds #[62.35000, 62.35000, 62.35000, 90.0000, 90.0000, 90.0000]
    ...
    # probe radius
    r_probe = modelp.r_probe #5.0
    angular_bin = modelp.angular_bin #2.0
    r_dist_bin = modelp.r_dist_bin #0.5
#    probe_theta = modelp.probe_theta #90.0
    r_power = modelp.r_power #2
    ...
    convergence_check_flag = modelp.convergence_check_flag #False

    fourbody = modelp.fourbody
    ...
    for j in np.arange(1, int(loops) + 1, 1):
        ...
        for i, atom in enumerate(cluster_asymm):
            f = open(output_path+ "_" + str(i) + "_three_body_array.dat","w")
            f.close()
            f = open(output_path+ "_" + str(i) + "_four_body_array.dat","w")
            f.close()
        
        processes = [mp.Process(target=molecular_n_body_problem_pool,
                                args=(i, atom, extended_atoms, r_probe, r_dist_bin, output_path, fourbody)) for i, atom in
        ...
        print("r_probe", modelp.r_probe, r_probe)
    ...
    if convergence_check_flag:
        ...
        finalise_histograms(output_path, angular_bin, r_dist_bin, r_power, convergence_loop)
    else:
        hist_path = output_path + "_unrefcor_loop_" + str(int(loops) + 1) + "_sum.dat"
        finalise_histograms(output_path, angular_bin, r_dist_bin, r_power, loops)
...
class model_padf():
    ...
    def __init__(self):
        ...
        self.project = "1al1/"
        self.xyz_name = "1al1_ex.xyz"  # the xyz file contains the cartesian coords of the crystal structure expanded to include r_probe
        self.cif_name = "1al1_edit.cif"  # the cif containing the asymmetric unit. This often needs to be edited for PDB CIFs hence '_edit' here
    
        #self.output_path = root + project + xyz_name[:-4]
    
        self.pool = mp.Pool(mp.cpu_count())  # change to set the number of processors
